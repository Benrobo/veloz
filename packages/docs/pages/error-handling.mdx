...
A custom error class that extends the default Error class called _`HttpException`_, this is used to throw custom errors in your codebase and can be found in the `/src/app/api/utils/exception.(ts / js)` file.
...
A custom error handler middleware called _`CatchError()`_ is used to handle all errors thrown from the server, this middleware can be found in the `src/app/api/utils/_error.(ts / js)` file. Notice the underscore before the `error` name, this is to prevent an override to default next.js custom `error` file.
...
All related zod schema validation are stored within the `src/app/api/utils/schema_validation.(ts / js)`
...
```js src/app/api/utils/schema_validation.(ts / js)
import zod from "zod";
...
How about sending response to client? well we've got you covered, we've created a utility class called _`sendResponse()`_ which is used to send response to the client, this function can be found in the `src/app/api/utils/sendResponse.(ts / js)` file. This class exposes two methods namely _`error()`_ and _`success()`_.
