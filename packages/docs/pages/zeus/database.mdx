...
Zeus makes support multiple `NoSQL` and `SQL` databases namely `MongoDB`, `MySQL`, `PostgreSQL`. You can choose any of them as per your need.
...
```bash mongodb_replica_servers/Dockerfile
FROM mongo:4
...
```bash .env
# mongo, postgres, mysql connection string
DATABASE_URL="..."
```
