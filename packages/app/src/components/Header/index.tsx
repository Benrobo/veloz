...
function Header() {
  const [tsvideo, setTsVideo] = useState(false);

  const toggleVideo = () => setTsVideo(!tsvideo);
    ...
    <FlexColStartCenter className="relative w-full h-auto pattern-bg border-b-solid border-b-[1px] border-b-gray-100/30 ">
      ...
      <FlexColCenter className="relative w-full h-full mt-[10em] md:mt-[8em] text-center">
        ...
        <FlexColCenter className="w-auto md:max-w-[70%]">
          ...
          <FlexRowCenterBtw>
            <button
              className="w-auto px-5 py-3 rounded-[30px] border-solid border-[3px] border-blue-100 transition-all hover:scale-[1] scale-[.90] "
      ...
      {tsvideo && (
        <TeaserVideoCont visible={tsvideo} close={() => toggleVideo()} />
      )}
...
function TeaserVideoCont({ visible, close }: TsVideoProps) {
  return (
    <FlexColCenter
      className={cn(
        "w-full h-screen bg-white-100/30 backdrop-blur fixed top-0 left-0 z-[1000]"
      )}
    >
      <FlexColStart
        className={cn(
          "w-full max-w-[400px] md:max-w-[70%] ",
      ...
      >
        <FlexRowEnd className="w-full">
          <button
            onClick={close}
            className="hover:scale-[1] scale-[.95] transition-all"
          >
            <X />
          </button>
        </FlexRowEnd>
        <FlexColCenter className="w-full h-[350px] md:h-[500px] scale-[.95] md:scale-[1] border-solid border-[6px] border-white-100/60 rounded-lg ">
          <iframe
            // width="560"
            // height="315"
            className="w-full h-full"
            src="https://www.youtube-nocookie.com/embed/yZGG0eYosg4?si=Z8qlBmZFXoIc_pgL"
